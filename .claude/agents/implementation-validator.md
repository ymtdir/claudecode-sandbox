...
description: 実装コードをプロジェクトスペック（PRD・機能設計・アーキテクチャ）と照合し品質を検証する。コード品質・テストカバレッジ・セキュリティ・パフォーマンスをチェック。実装完了後のスペック準拠確認に使用。
model: sonnet
---
...
| 症状 | 原因 | 対処 |
|------|------|------|
| `npm run lint` が失敗 | ESLint設定またはNode.jsバージョン不一致 | `node -v` と `package.json` の `engines` を確認 |
| `npm run typecheck` が失敗 | TypeScript型エラー | エラー出力を分析し、型定義の不足か実装ミスかを切り分け |
| `npm test` がタイムアウト | テスト環境の問題 | `npm test -- --watchAll=false` で非インタラクティブ実行 |
| スペックドキュメントが見つからない | `docs/` にドキュメント未作成 | `Glob('docs/**/*.md')` で存在確認、未作成なら検証をスキップ |